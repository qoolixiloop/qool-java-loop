package solutions.taskAddressBook.ui;
...
import week07.code.solutions.taskAddressBook.AddressBook;
import week07.code.solutions.taskAddressBook.Contact;
...
public class AddressBookFrame extends JFrame implements ActionListener {
	 ...
	 */
	public AddressBookFrame(AddressBook book) {
	...
	private void deleteContact() {
		...
		if(Pattern.matches("[0-9]", toBeDeleted)) {
			...
			Contact candidate = book.findById(id);
			if(candidate != null) {
				book.deleteContact(candidate);
	...
	private void searchForContact() {
		...
		Contact c = book.findByLastName(name);
		if(c != null) {
	...
	private void createComponents() {
		...
		// eigene Buttons, Textfelder, etc. hinzufügen können.
	...
	private Container createInputPanel() {
		...
		// Layout manager sorgen dafür, dass der Inhalt des
		// Fensters automatisch an dessen Grösse angepasst wird.
